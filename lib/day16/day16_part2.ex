defmodule Day16.Part2 do
  def call(type) do
    ...
    Enum.reduce(0..(grid_size - 1), 0, fn index, acc ->
      IO.inspect(index, label: "INDEX")

      top_down =
        move(%{x: index, y: 0, direction: :down}, :down, [], grid)
        |> Enum.map(&%{x: &1.x, y: &1.y})
        |> Enum.uniq()
        |> length()

      left_rigth =
        move(%{x: 0, y: index, direction: :right}, :right, [], grid)
        |> Enum.map(&%{x: &1.x, y: &1.y})
        |> Enum.uniq()
        |> length()

      down_up =
        move(%{x: index, y: grid_size - 1, direction: :up}, :up, [], grid)
        |> Enum.map(&%{x: &1.x, y: &1.y})
        |> Enum.uniq()
        |> length()

      right_left =
        move(%{x: grid_size - 1, y: index, direction: :left}, :left, [], grid)
        |> Enum.map(&%{x: &1.x, y: &1.y})
        |> Enum.uniq()
        |> length()

      max(Enum.max([top_down, left_rigth, down_up, right_left]), acc)
    end)
  end
